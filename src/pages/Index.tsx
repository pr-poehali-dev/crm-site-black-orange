          ...
          <div className="hidden md:flex items-center gap-1">
            {navItems.map((item) => (
              <button
                ...
                className={`nav-link px-4 py-2 rounded text-sm font-golos font-medium tracking-wide transition-all ${
                  activePage === item.id ? "active" : "text-gray-400 hover:text-white"
                }`}
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12">
      <div className="mb-12 animate-fade-in-up">
        ...
        <h1 className="font-oswald text-5xl font-bold uppercase mb-3">
        ...
        {steps.map((s, i) => (
          <div key={i} className="card-hover rounded-xl p-6 flex gap-6 animate-fade-in-up" style={{ backgroundColor: "#141414", animationDelay: `${i * 0.08}s` }}>
            <div className="flex-shrink-0">
              <div className="font-oswald text-4xl font-bold" style={{ color: "rgba(255,106,0,0.2)" }}>{s.step}</div>
            </div>
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-2">
                <div className="w-8 h-8 rounded flex items-center justify-center" style={{ backgroundColor: "rgba(255,106,0,0.1)" }}>
                  <Icon name={s.icon} size={16} style={{ color: "#FF6A00" }} />
                </div>
      ...
      <div className="rounded-xl p-8 animate-fade-in-up" style={{ backgroundColor: "#141414", border: "1px solid rgba(255,106,0,0.15)" }}>
        <div className="flex items-center gap-3 mb-6">
          <Icon name="AlertTriangle" size={22} style={{ color: "#FF6A00" }} />
          <h2 className="font-oswald text-2xl font-bold uppercase">Основные правила</h2>
        </div>
        <div className="grid md:grid-cols-2 gap-3">
          {rules.map((r, i) => (
            <div key={i} className="flex items-start gap-3 py-2">
              <div className="mt-0.5 w-5 h-5 rounded-full flex-shrink-0 flex items-center justify-center text-xs font-bold" style={{ backgroundColor: "rgba(255,106,0,0.2)", color: "#FF6A00" }}>
                {i + 1}
              </div>
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-12">
      <div className="flex items-start justify-between mb-10 animate-fade-in-up">
        <div>
          ...
          <h1 className="font-oswald text-5xl font-bold uppercase">
            <span className="gradient-text-orange">Форум</span>
          </h1>
        </div>
        <button className="mt-2 flex items-center gap-2 px-5 py-2.5 rounded font-oswald text-sm font-medium uppercase tracking-wider transition-all hover:scale-105" style={{ backgroundColor: "#FF6A00", color: "#0D0D0D" }}>
        ...
        {categories.map((cat, i) => (
          <div key={i} className="card-hover rounded-xl p-5 cursor-pointer animate-fade-in-up" style={{ backgroundColor: "#141414", animationDelay: `${i * 0.07}s` }}>
            <div className="flex items-start gap-4">
              <div className="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0" style={{ backgroundColor: `${cat.color}18` }}>
      ...
      <div className="rounded-xl overflow-hidden animate-fade-in-up" style={{ border: "1px solid #252525" }}>
        <div className="px-6 py-4 flex items-center gap-3" style={{ backgroundColor: "#141414", borderBottom: "1px solid #252525" }}>
          <Icon name="Clock" size={18} style={{ color: "#FF6A00" }} />
          <h2 className="font-oswald text-xl font-bold uppercase">Последние обсуждения</h2>
        </div>
        {recentPosts.map((post, i) => (
          <div key={i} className="px-6 py-4 flex items-center justify-between cursor-pointer transition-colors hover:bg-white/5" style={{ borderBottom: i < recentPosts.length - 1 ? "1px solid #1A1A1A" : "none" }}>
            <div className="flex items-center gap-4">
              <div className="w-8 h-8 rounded-full flex items-center justify-center font-oswald text-sm font-bold" style={{ backgroundColor: "rgba(255,106,0,0.15)", color: "#FF6A00" }}>
                {post.author[0]}
              </div>
              <div>
                <p className="font-golos text-sm font-medium">{post.title}</p>
                <p className="font-golos text-xs" style={{ color: "#555" }}>{post.author}</p>
              </div>
            </div>
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-12">
      <div className="mb-10 animate-fade-in-up">
        ...
        <h1 className="font-oswald text-5xl font-bold uppercase">
          <span className="gradient-text-orange">Интересное</span>
        </h1>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
        {articles.map((a, i) => (
          <div key={i} className="card-hover rounded-xl overflow-hidden cursor-pointer animate-fade-in-up" style={{ backgroundColor: "#141414", animationDelay: `${i * 0.07}s` }}>
            <div className="h-32 flex items-center justify-center relative" style={{ background: "linear-gradient(135deg, rgba(255,106,0,0.08), rgba(13,13,13,0.9))", borderBottom: "1px solid #252525" }}>
            ...
            <div className="p-5">
              ...
              <div className="flex items-center gap-1.5 text-xs font-golos" style={{ color: "#555" }}>
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      <div className="mb-10 animate-fade-in-up">
        ...
        <h1 className="font-oswald text-5xl font-bold uppercase">
          <span className="gradient-text-orange">Контакты</span>
        </h1>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-5 mb-12">
        {contacts.map((c, i) => (
          <div key={i} className="card-hover rounded-xl p-6 flex items-center gap-5 cursor-pointer animate-fade-in-up" style={{ backgroundColor: "#141414", animationDelay: `${i * 0.1}s` }}>
            <div className="w-14 h-14 rounded-xl flex items-center justify-center flex-shrink-0" style={{ backgroundColor: `${c.color}18`, border: `1px solid ${c.color}30` }}>
      ...
      <div className="rounded-xl p-8 animate-fade-in-up" style={{ backgroundColor: "#141414", border: "1px solid #252525" }}>
        <h2 className="font-oswald text-2xl font-bold uppercase mb-6 flex items-center gap-3">
          <Icon name="Mail" size={22} style={{ color: "#FF6A00" }} />
          Написать нам
        </h2>
        <div className="grid md:grid-cols-2 gap-4 mb-4">
          <div>
            <label className="font-golos text-xs block mb-2" style={{ color: "#666" }}>Имя</label>
            <input type="text" placeholder="Ваше имя" className="w-full px-4 py-3 rounded font-golos text-sm outline-none" style={{ backgroundColor: "#1A1A1A", border: "1px solid #303030", color: "#F0F0F0" }} />
          </div>
          <div>
            <label className="font-golos text-xs block mb-2" style={{ color: "#666" }}>Email</label>
            <input type="email" placeholder="your@email.com" className="w-full px-4 py-3 rounded font-golos text-sm outline-none" style={{ backgroundColor: "#1A1A1A", border: "1px solid #303030", color: "#F0F0F0" }} />
          </div>
        </div>
        <div className="mb-4">
          <label className="font-golos text-xs block mb-2" style={{ color: "#666" }}>Сообщение</label>
          <textarea rows={4} placeholder="Ваше сообщение..." className="w-full px-4 py-3 rounded font-golos text-sm outline-none resize-none" style={{ backgroundColor: "#1A1A1A", border: "1px solid #303030", color: "#F0F0F0" }} />
        </div>
        <button className="flex items-center gap-2 px-8 py-3 rounded font-oswald text-sm font-medium uppercase tracking-wider transition-all hover:scale-105 glow-orange" style={{ backgroundColor: "#FF6A00", color: "#0D0D0D" }}>
...
function SettingsPage() {
  ...
  const Toggle = ({ value, onChange }: { value: boolean; onChange: () => void }) => (
    <button
      onClick={onChange}
      className="relative inline-flex items-center w-12 h-6 rounded-full transition-all"
      style={{ backgroundColor: value ? "#FF6A00" : "#333" }}
    >
      <span
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-12">
      <div className="mb-10 animate-fade-in-up">
        ...
        <h1 className="font-oswald text-5xl font-bold uppercase">
          <span className="gradient-text-orange">Настройки</span>
        </h1>
      </div>
      ...
      <div className="space-y-5">
        {sections.map((sec, si) => (
          <div key={si} className="rounded-xl overflow-hidden animate-fade-in-up" style={{ border: "1px solid #252525", animationDelay: `${si * 0.1}s` }}>
            <div className="px-6 py-4 flex items-center gap-3" style={{ backgroundColor: "#141414", borderBottom: "1px solid #252525" }}>
              <Icon name={sec.icon} size={18} style={{ color: "#FF6A00" }} />
              <h2 className="font-oswald text-lg font-bold uppercase tracking-wide">{sec.title}</h2>
            </div>
            {sec.items.map((item, ii) => (
              <div key={ii} className="px-6 py-5 flex items-center justify-between transition-colors hover:bg-white/5" style={{ borderBottom: ii < sec.items.length - 1 ? "1px solid #1A1A1A" : "none" }}>
                <div>
                  <p className="font-golos text-sm font-medium mb-0.5">{item.label}</p>
                  <p className="font-golos text-xs" style={{ color: "#555" }}>{item.desc}</p>
                </div>
        ...
        <div className="rounded-xl overflow-hidden animate-fade-in-up delay-200" style={{ border: "1px solid #252525" }}>
          <div className="px-6 py-4 flex items-center gap-3" style={{ backgroundColor: "#141414", borderBottom: "1px solid #252525" }}>
            <Icon name="Globe" size={18} style={{ color: "#FF6A00" }} />
            <h2 className="font-oswald text-lg font-bold uppercase tracking-wide">Язык</h2>
          </div>
          <div className="px-6 py-5 flex items-center justify-between">
            <div>
              <p className="font-golos text-sm font-medium mb-0.5">Язык интерфейса</p>
              <p className="font-golos text-xs" style={{ color: "#555" }}>Выбери язык сайта</p>
            </div>
            <select
              value={language}
              onChange={(e) => setLanguage(e.target.value)}
              className="px-4 py-2 rounded font-golos text-sm outline-none cursor-pointer"
              style={{ backgroundColor: "#252525", color: "#F0F0F0", border: "1px solid #333" }}
            >
        ...
        <div className="rounded-xl p-6 animate-fade-in-up delay-300" style={{ border: "1px solid rgba(244,67,54,0.2)", backgroundColor: "rgba(244,67,54,0.03)" }}>
          <h2 className="font-oswald text-lg font-bold uppercase tracking-wide mb-4 flex items-center gap-3" style={{ color: "#F44336" }}>
            <Icon name="AlertTriangle" size={18} style={{ color: "#F44336" }} />
            Опасная зона
          </h2>
          <div className="flex items-center justify-between">
            <div>
              <p className="font-golos text-sm font-medium mb-0.5">Удалить аккаунт</p>
              <p className="font-golos text-xs" style={{ color: "#555" }}>Это действие необратимо</p>
            </div>
            <button className="px-5 py-2 rounded font-oswald text-sm font-medium uppercase tracking-wider transition-all hover:scale-105" style={{ backgroundColor: "rgba(244,67,54,0.15)", color: "#F44336", border: "1px solid rgba(244,67,54,0.3)" }}>
              Удалить
            </button>
          </div>
        </div>
